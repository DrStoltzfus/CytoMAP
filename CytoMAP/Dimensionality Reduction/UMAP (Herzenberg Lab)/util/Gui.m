...
classdef Gui
    ...
    methods (Static)
        ...
        function otherPlotHs=PlotClusters(ax3D, data, clusterIds, ...
                xLabel, yLabel, zLabel, doLegend, ax2D)
            if nargin<8
                ax2D=[];
                if nargin<7
                    doLegend=true;
            ...
            numClusters=sum(clues>=0);
            numClues=length(clues);
            cnts=zeros(1,numClues);
            for i=1:numClues
                clue=clues(i);
                cnts(i)=sum(clusterIds==clue);
            end
            [~,ranks]=sort(cnts);
            ...
            if isempty(ax3D)
                ...
                set(fig2, 'name', ...
                    [num2str(numClusters) ' clusters found`...']);
            ...
            if ~isempty(ax2D)
                otherPlotHs=zeros(1,numClues);
            else
                otherPlotHs=[];
            end
            plotHs=zeros(1,numClues);
            names=cell(1, numClues);
            for i=1:numClues
                ranking=find(i==ranks,1);
                clue=clues(i);
                if clue<1
                    clr=[.2 .2 .2];
                else
                    clr=jets(ranking*step, :);
                end
                l=clusterIds==clue;
                %clr2=floor(clr*256);
                %fprintf('Cluster #%d: id=%d has %d %d events, ranking=%d rgb=%d.%d.%d\n', ...
                %    i, clues(i), cnts(i), sum(l), ranking, clr2(1), clr2(2), clr2(3));
                if n_components>2
                    if clue<1 || doLegend
                        plotHs(i)=plot3(ax3D, data(l,1), data(l,2), data(l,3), '.', ...
                        ...
                        if clue<1
                            ...
                            names{i}=['Cluster ' num2str(clue)];
                        end
                        if ~isempty(ax2D)
                            otherPlotHs(i)=plot(ax2D, data(l,1), ...
                                data(l,2), '.', ...
                                'visible', 'off',...
                                'markerSize', 5, ...
                                'lineStyle', 'none', ...
                    ...
                    for j=1:denominator
                        ...
                        fprintf('%d of %d', sum(l2), cnts(i));
                        fprintf('\n');
                    end
                    
                else
                    plot(ax3D, data(l,1), data(l,2), '.', ...
            ...
            if n_components<3
                ProbabilityDensity2.Draw(ax3D, data, true, false, false);
            end
            if doLegend
                if isempty(ax2D)
                    Gui.Legend(plotHs, names, [], -0.01, 0.061, true, ...
                        sum(cnts), [], [])
                else
                     Gui.Legend([plotHs;otherPlotHs], names, [], -0.01,...
                         0.061, true, sum(cnts), [], [])
                end
            end
        end
...
%                disp('java already');
        ...
        function [HL, fncMotion, btns, sortI]=Legend(plotHs, names, ...
            ...
            if ~javaWnd
                ...
                HL.ItemHitFcn=@(h,e)Gui.LegendClick(e, legendHs, ...
                    idxsWithName, plotHs, cnts2, otherPlotHs);
            else
                for i=1:nNames
                    idx=idxsWithName(i);
                    cnt=cnts(idx);
                    ...
                    names{i}=['<html>' str names{i} ' <font color="blue">' ...
                ...
                [outerPnl, ~, btns,sortGui]=...
                    ...
                    @(h,e, i)Gui.JavaLegendClick(i,sortI,idxsWithName, ...
                    plotHs, cnts2, h),true,Html.Wrap(['<i>Deselecting '...
                ...
                if curOver ~= over
                    ...
                    if over>0
                        ...
                        if nameIdx>0
                            ...
                            if isempty(overH)
                                overH=text(normX+.03, normY+.03, nm, ...
        ...
        function LegendClick(event, legendHs, idxsWithName, plotHs,...
                nums, otherPlotHs)
            if nargin<6
                otherPlotHs=[];
            end
            ...
            doOthers;
            function doOthers
                if ~isempty(otherPlotHs)
                    R=size(otherPlotHs,1);
                    for r=1:R
                        plotH=otherPlotHs(r, idx);
