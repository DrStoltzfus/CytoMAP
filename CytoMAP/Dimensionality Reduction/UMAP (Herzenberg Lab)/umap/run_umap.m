...
if nargout>1
        ...
        if isempty(data)
            ...
            if show
                if ~isempty(clusterIds)
                    Gui.PlotClusters([], data, clusterIds, xLabel, ...
                        yLabel, zLabel);
                end
            end
        end
            ...
            if args.n_components>2
                ...
                if labelCols>0
                    ...
                    if isprop(umap, 'supervisors') ...
                        ...
                        if nargin>1 && lastCall
                            [labels, labelMap]=...
                                umap.supervisors.supervise(data, true, true);
                            ProbabilityDensity2.DrawLabeled(curAxes, data,...
                                labels, labelMap, true, true, [], [], -0.01,...
                                0.061, true,[])
                        else
    ...
    function p=parseArguments(varargin)
        ...
        defaultMethod='Java';
        expectedMethod={'Java', 'C', 'C vectorized', 'MATLAB', 'MATLAB vectorized', 'MATLAB experimental'};
