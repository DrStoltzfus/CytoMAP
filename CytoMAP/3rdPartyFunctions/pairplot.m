function pairplot(meas, label, group, colors)
    ...
    %
    % Original code by Ryosuke Takeuchi 2016/12/22 - 2017-01-24
    mode = 'histogram';
    groupName = unique(group, 'stable');
    ngroup = length(groupName);
    for i = 1:size(meas, 2)
        for j = 1:size(meas, 2)
            subAx = subplot(size(meas, 2),size(meas, 2),sub2ind([size(meas, 2) size(meas, 2)], i, j)); 
            if i==1
                ylabel(label{j}, 'rotation', 0, 'VerticalAlignment','middle', 'HorizontalAlignment','right');
            end
            if j==size(meas, 2)
                xlabel(label{i}, 'rotation', 20, 'VerticalAlignment','middle', 'HorizontalAlignment','right');
            end
            hold on;
            if i==j
                ...
                for g = 1:ngroup				
                    switch mode
                        case 'histogram'
                            ...
                            xlim([bin(1) bin(end)])
                        case 'bar'
                            [counts, ~] = histc(meas(strcmp(group, groupName{g}), i), bin);
